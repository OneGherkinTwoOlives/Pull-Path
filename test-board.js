...
const TEST_BOARD_SUFFIX = "-magnetic-end-test";
...
function getBoardStateProjectId() {
  return `${getProjectIdFromLocation()}${TEST_BOARD_SUFFIX}`;
}

function getStorageKey() {
  return `board-state-${getBoardStateProjectId()}`;
}

function getViewStorageKey() {
  return `board-view-${getBoardStateProjectId()}-${currentUserKey}`;
}
...
async function saveState() {
  try {
    ...
    if (window.TSData?.saveBoardState) {
      await window.TSData.saveBoardState(getBoardStateProjectId(), serialized);
    } else {
...
async function loadState() {
  try {
    if (window.TSData?.fetchBoardState) {
      return await window.TSData.fetchBoardState(getBoardStateProjectId());
    }
...
function planningWeekInsertStartX(laneId) {
  ...
  state.notes.forEach((note) => {
    ...
    const weekIndex = planningWeekIndexFromStartX(noteStartX(note));
    if (weekIndex === null) {
      return;
    }
...
function setupRealtimeSync() {
  ...
  const projectId = getBoardStateProjectId();
  if (!projectId) return;
